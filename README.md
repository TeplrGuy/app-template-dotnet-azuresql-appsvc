...
1. What symptoms above is the application seeing?
   + Check Application Insights Performance blade, filter on Roles to identify which app is seeing the slowness.
   + Check Failures blade to understand the symptoms a bit more and the type of 5xx status codes being seen.
2. Are there multiple applications running on the App Service Plan? Single instance? Production SKU?
   + If not using a production SKU, consider scaling up and monitor
   + If running on a single instance, consider scaling out and monitor. Note: if the issue is SNAT it will likely bubble up again as traffic increases.
   + If there are too many apps in the same App Service Plan, consider splitting them up. Example in this scenario we can host the frontend on its own ASP and do the same for the backend service to improve app level resiliency and performance.
3. Is the application making any outbound calls that is slow or failing ?
   + Check Application Map and Performance blade In App Insights to identify this quickly
     + here you want to look for patterns (i.e similar timeout duration, 5xx errors, 4xx(i.e client gone), socket exceptions, etc)
4. How is CPU and Memory usage? 
...
Following the scoping steps, you should be able to now have a better idea of the issue. We will run through the below TSG to help us identity and confirm if the issue is truly a SNAT issue.

#### Important Detectors
* SNAT Port Exhaustion 
    + Find if there is SNAT Port Exhaustion for Web App
* SNAT Failed Connection Endpoints 
    + Identify the top SNAT ports consuming destination endpoints the app was connecting to. These endpoints can help identify potential connection leaks and fix SNAT port exhaustion in the app.
* Web App Slow / Slow Requests 
    + Show basic web app information and detects if there is high load avg, zombie processes, low swap space or high I/O wait
* CPU usage / Linux CPU Drill Down / Memory Usage/ Linux Memory Drill Memory Drill Down
    + Identity which container causing high CPU or memory.
* App Service Plan Density Check 
    + Check the amount of active apps running on a single ASP.
* Web App Restarted
    + Check to see if the symptoms caused the Web App to restart. Usually high CPU and High Memory for a few minutes will cause the platform [proactive auto heal](https://azure.github.io/AppService/2017/08/17/Introducing-Proactive-Auto-Heal.html) to kick in.

#### Steps to identify root cause of SNAT

Now that we have a good idea the issue is SNAT based on the symptoms we are seeing, follow the steps below to understand the issue.

1. Check SNAT Port Exhaustion Detector to identify spikes. 
    If you don't see spikes and see consistent # of established connections over a few hours indicates the code is likely using connection pooling. if we see spikes and or pending/failed port usage then we will need to repro and take a .Net profiler trace to identify the root cause.
    ![alt text](image.png)
...
2. Check Application Insights for common errors
    ...
    From the looks of it, the SQL call is quick but there is a slow external call which was canceled after sometime. This is a good place to start looking in the code to see if the behavior makes sense. What we are seeing could just be a symptom as well.
    ![alt text](image-4.png)
    
    TIP: If Application Insights profiler is enabled by default, check to see if it is able to sample/capture slowness or hot spots that can tell us where the issue is.

3. Take a .Net profiler trace to identity any slow requests or endpoints. 
    To take a trace, visit the web app -> Diagnose and solve problems -> Diagnostic Tools -> Collect .NET Profiler Trace -> follow instructions to take the trace and the platform will analyze for you. 
    Example: 
